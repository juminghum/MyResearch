...
class DotProductOperands extends Bundle {
  val a1 = UInt(32.W)
  val a2 = UInt(32.W)
  val b1 = UInt(32.W)
  val b2 = UInt(32.W)
...
class DotProductResult extends Bundle {
  val c = UInt(32.W)
}

class MyDotProduct extends Module {
  val io = IO(new Bundle{
    val req = Flipped(new DecoupledIO( new DotProductOperands ))
    val resp = new DecoupledIO( new DotProductResult )
  ...
  io.req.ready := true.B
  // データを捕まえる
  val a1 = RegEnable(io.req.bits.a1, io.req.valid)
  val a2 = RegEnable(io.req.bits.a2, io.req.valid)
  val b1 = RegEnable(io.req.bits.b1, io.req.valid)
  val b2 = RegEnable(io.req.bits.b2, io.req.valid)
  
  
  when(io.req.valid && !state) {
    state := true.B
  }
  
  
  io.resp.bits.c := a1 + a2 + b1 + b2 //a1 * a2 + b1 * b2
  
  io.resp.valid := false.B // Default
  when(state) {
  ...
  io.busy := state
  // io.resp.ready 待って
  when(io.resp.ready && state) {
    state := false.B
  }
}

class  FusedDotProductExample(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(opcodes) { // Lazymodule
  override lazy val module = new FusedDotProductModuleImp(this)
}

class FusedDotProductModuleImp(outer: FusedDotProductExample)(implicit p: Parameters) extends LazyRoCCModuleImp(outer) // Lazemodule imp
    with HasCoreParameters {
/*
  val cmd = io.cmd //Queue(io.cmd)
  
  val rd = RegEnable(cmd.bits.inst.rd, cmd.fire()) // rd の　アドレスだけ
  
  val m = Module(new MyDotProduct)
  println("Including the DotProduct")
  
  // リクエスト
  m.io.req.valid := cmd.valid
  cmd.ready := m.io.req.ready
  m.io.req.bits.a1 := cmd.bits.rs1(63, 32)
  m.io.req.bits.a2 := cmd.bits.rs1(31, 0)
  m.io.req.bits.b1 := cmd.bits.rs2(63, 32)
  m.io.req.bits.b2 := cmd.bits.rs2(31, 0)
  
  //　レスポンス　（返信）
  io.resp.valid := m.io.resp.valid
  m.io.resp.ready := io.resp.ready
  io.resp.bits.rd := rd // 同じ
  io.resp.bits.data := m.io.resp.bits.c // 0.U(32.W) and C
  io.busy := m.io.busy
  io.interrupt := false.B
  */
  
  val FDP = Module(new FusedDotProduct)
  FDP.io.enable := Bool(true)//io.cmd.fire()
  val funct = io.cmd.bits.inst.funct
  val FDPout = Reg(io.resp.bits.data)
  val req_rd = Reg(io.resp.bits.rd)
  val doAdd = funct === UInt(0)
  val doSub = funct === UInt(1)
  val do_D0_Add = funct === UInt(2)
  val do_D0_Sub = funct === UInt(3)
  val do_B0_Add = funct === UInt(4)
  val do_B0_Sub = funct === UInt(5)
  val do_B0_D0_Add = funct === UInt(6)
  val do_B0_D0_Sub = funct === UInt(7)
  val exception = FDP.io.EXCEPTION
  val do_valid = doAdd || doSub || do_D0_Add || do_D0_Sub || do_B0_Add || do_B0_Sub || do_B0_D0_Add || do_B0_D0_Sub

  val first_stage :: second_stage :: third_stage :: fourth_stage :: Nil = Enum(Bits(), 4)
  val state = Reg(init = first_stage)

  io.cmd.ready := (state === first_stage)
  //FDPout := FDP.io.OUT

  when (io.cmd.fire() && do_valid) {
    FDP.io.INA := io.cmd.bits.rs1(63, 32)
    FDP.io.INB := io.cmd.bits.rs1(31, 0)
    FDP.io.INC := io.cmd.bits.rs2(63, 32)
    FDP.io.IND := io.cmd.bits.rs2(31, 0)
	FDP.io.Op_Sel := funct
	state := second_stage
	req_rd := io.cmd.bits.inst.rd
	//printf("FusedDotProduct: Command Received.\n")
  }

  when (state === second_stage) { state := third_stage }

  when (state === third_stage) { state := fourth_stage }

  when (state === fourth_stage) {// && io.resp.fire()) { 
	state := first_stage
	FDPout := FDP.io.OUT
	//printf("FusedDotProduct: Finished.\n")
  }
  // PROC RESPONSE INTERFACE
  io.resp.valid := (state === fourth_stage)
  io.resp.bits.rd := req_rd
  //io.resp.bits.data := FDPout
  io.resp.bits.data := FDPout
  io.busy := (state =/= first_stage)
  io.interrupt := Bool(false)
