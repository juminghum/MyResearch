...
static void IN_INIT_SECTION __check_fromhost()
{
...
static void IN_INIT_SECTION __set_tohost(uintptr_t dev, uintptr_t cmd, uintptr_t data)
{
  while (tohost)
    __check_fromhost();
...
static void IN_INIT_SECTION do_tohost_fromhost(uintptr_t dev, uintptr_t cmd, uintptr_t data)
{
  spinlock_lock(&htif_lock);
...
void IN_INIT_SECTION htif_syscall(uintptr_t arg)
{
  do_tohost_fromhost(0, 0, arg);
}

void IN_INIT_SECTION htif_console_putchar(uint8_t ch)
{
#if __riscv_xlen == 32
...
void IN_INIT_SECTION htif_poweroff()
{
  while (1) {
    fromhost = 0;
    tohost = 1;
    *((unsigned int*)0x30000000) = 123456789;
    __asm__ ("ebreak");
  }
}
