...
class ClosePathLogic extends Module {
  val io = IO(new Bundle {
    ...
    val NORM_SHIFT = Output(SInt(6.W))
    //val Correcting = Output(UInt(1.W))
  })
  ...
  //AB_R_inv := Aligned_AB_R
  //AB_S_inv := Aligned_CD_R
  ...
  //FTC51_A.io.B := (AB_S_inv(49, 0) ## zero_vec(0))
  FTC51_A.io.B := AB_S_inv
  FTC51_A.io.C := Aligned_CD_R
  //FTC51_A.io.D := (Aligned_CD_S(49, 0) ## zero_vec(0))
  ...
  //FTC51_B.io.B := (Aligned_AB_S(49, 0) ## zero_vec(0))
  FTC51_B.io.B := Aligned_AB_S
  FTC51_B.io.C := CD_R_inv
  //FTC51_B.io.D := (CD_S_inv(49, 0) ## zero_vec(0))
  FTC51_B.io.D := CD_S_inv
  CSA_B_R := FTC51_B.io.R
  CSA_B_S := FTC51_B.io.S

  val sig_comp = Wire(UInt(1.W))
  sig_comp := CSA_A_R(50) & (CSA_A_S(49) || CSA_A_S(50))
  io.SIGNIF_COMP := sig_comp

  val Norm_R = Wire(UInt(51.W))
  val Norm_S = Wire(UInt(51.W))

  Norm_R := Mux(sig_comp === 0.U, CSA_B_R, CSA_A_R)
  Norm_S := Mux(sig_comp === 0.U, CSA_B_S, CSA_A_S)

  val Norm_shift = Wire(UInt(7.W))
  ...
  Norm_shift := ("b0".U ## LZA.io.SHIFT_VALUE)
...
*/
  Norm_R_1stage := Mux(Norm_shift(5) === 1.U, Norm_R(18, 0) ## zero_vec, Norm_R)
  Norm_S_1stage := Mux(Norm_shift(5) === 1.U, Norm_S(18, 0) ## zero_vec, Norm_S)

  Norm_R_2stage := Mux(Norm_shift(4) === 1.U, Norm_R_1stage(34, 0) ## zero_vec(31,16), Norm_R_1stage)
  Norm_S_2stage := Mux(Norm_shift(4) === 1.U, Norm_S_1stage(34, 0) ## zero_vec(31,16), Norm_S_1stage)

  Norm_R_3stage := Mux(Norm_shift(3) === 1.U, Norm_R_2stage(42, 0) ## zero_vec(15,8) , Norm_R_2stage)
  Norm_S_3stage := Mux(Norm_shift(3) === 1.U, Norm_S_2stage(42, 0) ## zero_vec(15,8), Norm_S_2stage)

  Norm_R_4stage := Mux(Norm_shift(2) === 1.U, Norm_R_3stage(46, 0) ## zero_vec(7,4), Norm_R_3stage)
  Norm_S_4stage := Mux(Norm_shift(2) === 1.U, Norm_S_3stage(46, 0) ## zero_vec(7,4), Norm_S_3stage)

  Norm_R_5stage := Mux(Norm_shift(1) === 1.U, Norm_R_4stage(48, 0) ## zero_vec(3,2), Norm_R_4stage)
  Norm_S_5stage := Mux(Norm_shift(1) === 1.U, Norm_S_4stage(48, 0) ## zero_vec(3,2), Norm_S_4stage)

  Norm_R_6stage := Mux(Norm_shift(0) === 1.U, Norm_R_5stage(49, 0) ## zero_vec(1), Norm_R_5stage)
  Norm_S_6stage := Mux(Norm_shift(0) === 1.U, Norm_S_5stage(49, 0) ## zero_vec(1), Norm_S_5stage)
  ...
  //io.NORM_SHIFT := Norm_shift.asSInt() + Correction.asSInt() - 1.S
  io.NORM_SHIFT := (Norm_shift + Correction).asSInt() - 1.S
  //io.Correcting := Correction
}
...
class MantissaAlignment extends Module {
  ...
  switch (io.EXP_DIFF) {
    //is(-2.S) {
    is(0x3fe.S) {
      AB_S_buf := zero_vec(1, 0) ## io.INAB_S(37, 0) ## zero_vec(10, 0)
      AB_R_buf := zero_vec(1, 0) ## io.INAB_R ## zero_vec(0)
      CD_S_buf := io.INCD_S(37, 0) ## zero_vec(12, 0)
      CD_R_buf := io.INCD_R ## zero_vec(2, 0)
    }
    //is(-1.S) {/*
    is(0x3ff.S) {
      AB_S_buf := zero_vec(0) ## io.INAB_S(37, 0) ## zero_vec(11, 0)
...
/*
    ...
    is(2.S) {
      AB_S_buf := io.INAB_S(37, 0) ## zero_vec(12, 0)
      AB_R_buf := io.INAB_R ## zero_vec(2, 0)
      CD_S_buf := zero_vec(1, 0) ## io.INCD_S(37, 0) ## zero_vec(10, 0)
